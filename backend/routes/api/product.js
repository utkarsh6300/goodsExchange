...
router.post(
  ...
  [
    ...
    check("price", "Product price must be greater than or equal to zero").isInt(
      { min: 0 }
    ),
...
router.put(
  ...
  [
    ...
    check("quantity", "Quantity must be greater than or equal to zero").isInt({
      min: 0,
    }),
  ],
  async (req, res) => {
    ...
    try {
      ...
      if (product.owner.toString() !== req.user.id) {
        return res
          .status(403)
          .json({ msg: "You are not the owner of this product" });
      }
...
router.get("/update-quantity/:productId", authMiddleware, async (req, res) => {
  const productId = req.params.productId;
